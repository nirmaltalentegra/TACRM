...
/*
 * Function name : exportExcelData
 * Purpose : helper for export excel
 * Created by: Asmitha
...
function exportExcelData($records)
    ...
    function currency_format($currency,$position,$amount)
    {
        $return =   '';
        switch ($position) {
            case 'left': $return = $currency.' '.$amount;
                            break;
            case 'right': $return = $amount.' '.$currency;
                            break;            
    ...
    function get_status_id($status_name)
    {
        $CI = &get_instance();
        $CI->db->select("*");        
        $CI->db->where('status', $status_name);
        $query = $CI->db->get('status');
        if ($query->num_rows() > 0) {
            $data = $query->result_array();
    ...
    function get_staff_lead_status_count($staff_id,$status)
    {
        $CI = &get_instance();
        $CI->db->select("*");
        $CI->db->where('assigned_staff_id', $staff_id);
        $CI->db->where('status', $status);
        $query = $CI->db->get('leads');
        return $query->num_rows();
    }
    
    function get_staff_lead_source_count($staff_id,$source_id)
    {
        $CI = &get_instance();
        $CI->db->select("*");
        $CI->db->where('assigned_staff_id', $staff_id);
        $CI->db->where('source_id', $source_id);
        $query = $CI->db->get('leads');
        return $query->num_rows();
    }
    
    function get_source_lead_status_count($source_id,$status)
    {
        $CI = &get_instance();
        $CI->db->select("*");
        $CI->db->where('status', $status);
...
function get_user_id_by_mobile($mobile_no) {
    $CI = &get_instance();
    $CI->db->select("*");
    $CI->db->join('users','users.id = user_mobile.user_id AND users.default_mobile_no = user_mobile.id ','LEFT');
...
function get_user_id_by_email($email_id) {
    ...
    $CI->db->join('users','users.id = user_email.user_id AND users.default_email_id = user_email.id','LEFT');
...
/*
 * Function name : get_all_staff_details
 * Purpose : get_all_staff_details
 * Created by: Vivek
...
function get_all_staff_details() {
	$CI = &get_instance();
	$CI->db->order_by('staff_id', 'DESC');
    return $CI->db->get('staff')->result_array();
}
