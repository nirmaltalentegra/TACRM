...
class Dept_model extends CI_Model {
    ...
    function get_staff_by_dept_id($dept_id,$staff_assigned_id = NULL) {
        ...
        if($staff_assigned_id > 0) {
            $this->db->where('arbac_users.id', $staff_assigned_id);
        }
	...
	function get_dept_by_staff($staff_id)
	{
        ...
        function get_all_staff_by_dept() {
        $this->db->join('staff', 'staff.dept_id = department.id', 'LEFT');
        $query = $this->db->get($this->table);
        //echo "<br> str ".$this->db->last_query();exit;
        $result = $query->result_array();
