...
class Batches extends APP_Controller
{
	...
	public function read($id)
	{
		...
		if ($row) {
			...
			$row_faculty = $this->Common_model->get_details_dynamically('profiles', 'id', $row->faculty_id, 'id', $oder_by = NULL);
	...
	public function create_action()
	{
		...
		if ($this->form_validation->run() == FALSE) {
			...
			$data = array(
				...
				'week_days' => ($this->input->post('week_days', TRUE)) ? implode(',', $this->input->post('week_days', TRUE)) : '',
			...
			if (!$this->Batches_model->insert($data)) {
	...
	public function update_action()
	{
		...
		if ($this->form_validation->run() == FALSE) {
			...
			if (!$this->Batches_model->update($this->input->post('batch_id', TRUE), $data)) {
				$this->session->set_flashdata('error', 'Same name cannot be repeated.');
			} else {
	...
	public function delete($id)
	{
		...
		if ($row) {
			...
			if (!$this->Batches_model->delete($id)) {
				$this->session->set_flashdata('error', 'Batch already exist in its subsections');
			} else {
